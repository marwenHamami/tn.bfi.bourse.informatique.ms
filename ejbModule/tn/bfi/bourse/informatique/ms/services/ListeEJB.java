...
public class ListeEJB implements ListeEJBRemote, ListeEJBLocal {
	...
	public List<Liste> findAll() {
		return entityManager.createQuery("SELECT l FROM Liste l", Liste.class)
				.getResultList();
	...
	public List<Liste> findByClient(Client client) {
		return entityManager
				.createQuery("SELECT l FROM Liste l WHERE l.user=:user",
						Liste.class).setParameter("user", client)
				.getResultList();
	}
